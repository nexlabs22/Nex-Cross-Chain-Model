...
contract DeployFeeReceiver is Script {
    function run() external {
        ...
        if (keccak256(bytes(targetChain)) == keccak256("sepolia")) {
            ...
            crossChainFactoryStorage = vm.envAddress("ARBITRUM_SEPOLIA_CROSS_CHAIN_INDEX_FACTORY_STORAGE_PROXY_ADDRESS");
        } else if (keccak256(bytes(targetChain)) == keccak256("arbitrum_mainnet")) {
            chainlinkToken = vm.envAddress("ETHEREUM_CHAINLINK_TOKEN_ADDRESS");
            ccipRouter = vm.envAddress("ETHEREUM_CCIP_ROUTER_ADDRESS");
