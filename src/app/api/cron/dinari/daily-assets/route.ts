...
export async function GET(request: NextRequest) {
  ...
  const matchedStocks: (DailyAsset | null)[] = dinariAssets.map((asset) => {
    ...
    if (!quote || !asset.dinari?.stock.symbol || !asset.dinari?.stock.name) {
      return null
    }

    return {
      name: asset.dinari?.stock.name.toLowerCase(),
      ticker: asset.dinari?.stock.symbol.toUpperCase(),
  ...
  const filteredMongoDbData = matchedStocks.filter(
    (item) => item?.name && item?.ticker
  )

  await uploadStocksToDailyAssets(filteredMongoDbData, dailyAssetsCollection)
