...
export async function GET() {
  try {
    ...
    for (const token of nexTokensArray.filter(
      (token) => token.symbol === "ARBEI"
    )) {
      const todayDoc = await getDocument(collection, token)

      const storageContract = getContract({
        address: token?.tokenAddresses?.Arbitrum?.Mainnet?.storage
          ?.address as Address,
        chain: arbitrum,
        client: client,
      })
      const indexContract = getContract({
        address: token?.tokenAddresses?.Arbitrum?.Mainnet?.token
          ?.address as Address,
        chain: arbitrum,
        client: client,
      })
    ...
    return NextResponse.json({
      status: 200,
      message: "Thirdweb on-chain index data updated.",
      //   dataToUpload,
    })
  } catch (err) {
