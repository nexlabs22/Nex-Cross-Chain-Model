...
import { exchangeAddresses } from '@/constants/contractAddresses'
...
export async function GET() {
	...
	try {
		...
		cryptoArray.forEach((pair: string) => {
			...
			const weight = Number(marketCap) / sumOfMarketCap
			const selectedExchange = typeof minTradeValue.bitfinex.value === 'number' ? 'bitfinex' : typeof minTradeValue.bybit.value === 'number' ? 'bybit' : 'none'

			if (selectedExchange !== 'none') {
				if (!!map.get(selectedExchange)) {
					let cumulatveWeight = map.get(selectedExchange) || 0
					cumulatveWeight += weight;
					map.set(selectedExchange,cumulatveWeight)
				}else{
					map.set(selectedExchange,weight)
				}
			}
		})

		const dataToSend : { addresses: string[], marketShares: number[]} = {addresses:[],marketShares:[]}
