...
const TopIndexData = () => {
	const { anfiIndexObject, cr5IndexObject, mag7IndexObject, arbIndexObject, CR5UnderLyingAssets, ANFIUnderLyingAssets, MAG7UnderLyingAssets, ARBInUnderLyingAssets, IndicesWithDetails } =
		useDashboard()

	const { handleCopyIndexDetailsFunction } = usePortfolio()
	...
	const [selectedIndexWeights, setSelectedIndexWeights] = useState(
		defaultIndex == 'ANFI' ? ANFIUnderLyingAssets : defaultIndex == 'CRYPTO5' ? CR5UnderLyingAssets : defaultIndex == 'MAG7' ? MAG7UnderLyingAssets : ARBInUnderLyingAssets
	)
						...
						>
							...
							<Stack width={{ sm: '48%', lg: '35%' }} height={'100%'} direction={'column'} alignContent={'start'} justifyContent={'start'} padding={4}>
								...
								<Stack direction={'row'} alignItems={'center'} justifyContent={'space-between'} width={'100%'} height={'fit-content'} marginTop={'1.2rem'} marginBottom={'0.8rem'}>
									<Stack direction={'column'} width={'50%'} maxWidth={'50%'} alignItems={'start'} justifyContent={'space-between'} gap={1}>
										...
										>
											{/* {selectedIndex?.mktCap.toFixed(2).toFixed(2) + " " + selectedIndex?.shortSymbol} */}${selectedIndex?.mktCap.toLocaleString('en-US', { maximumFractionDigits: 2 })}
										</Typography>
									</Stack>
									<Stack direction={'column'} width={'50%'} maxWidth={'50%'} alignItems={'start'} justifyContent={'space-between'} gap={1}>
										<Stack direction={'row'} alignItems={'center'} justifyContent={'start'} gap={1}>
								...
								<Stack direction={'row'} alignItems={'center'} justifyContent={'space-between'} width={'100%'} height={'fit-content'} marginY={'0.8rem'}>
									<Stack direction={'column'} width={'100%'} alignItems={'start'} justifyContent={'space-between'} gap={1}>
										...
										<Stack direction={'row'} alignItems={'center'} justifyContent={'start'} gap={1}>
											...
											<CopyToClipboard text={selectedIndex?.tokenAddress as string} onCopy={handleCopyIndexDetailsFunction}>
												<BiCopy color="#5E869B" size={15} className="scale-150 xl:scale-100" />
											</CopyToClipboard>
										</Stack>
									</Stack>
								</Stack>
							...
							<Stack width={{ xs: '48%', lg: '35%' }} height={'100%'} direction={'column'} alignContent={'start'} justifyContent={'start'} padding={4}>
								...
								<Grid
									...
									rowSpacing={2}
									paddingRight={{ sm: 1 }}
									maxHeight={{ sm: '38vh', md: '38vh', lg: '40vh' }}
									sx={{
										overflowY: 'auto',
										overflowX: 'hidden',
									}}
						...
						<Stack width={'100%'} height={'fit-content'} paddingX={'1.2rem'} marginY={'1rem'}>
							...
							<Stack width={'100%'} height={'fit-content'} direction={'row'} alignItems={'center'} justifyContent={'start'} gap={6} marginTop={'1.2rem'}>
								{selectedIndex?.symbol != 'ANFI' ? (
									<Link
										href=""
										className="h-fit w-fit flex flex-row items-start justify-start"
										onClick={(e) => {
											e.preventDefault()
									...
									>
										<Stack direction={'row'} alignItems={'center'} justifyContent={'start'} gap={1}>
											<Image
												src={anfiIndexObject && anfiIndexObject.logo ? anfiIndexObject?.logo : ''}
												alt=""
												height={60}
												width={60}
												className="mr-2 border border-[#E8BB31] rounded-full "
											/>
								...
								{selectedIndex?.symbol != 'MAG7' ? (
									<Link
										...
										onClick={(e) => {
											e.preventDefault()
											changeDefaultIndex('MAG7')
										}}
									>
										<Stack direction={'row'} alignItems={'center'} justifyContent={'start'} gap={1}>
											<Image
												src={mag7IndexObject && mag7IndexObject.logo ? mag7IndexObject?.logo : ''}
												alt=""
												height={60}
												width={60}
												className="mr-2 border border-[#D67DEC] rounded-full "
											/>
											<Stack direction={'column'} alignItems={'start'} justifyContent={'start'}>
												<Typography
													variant="subtitle1"
