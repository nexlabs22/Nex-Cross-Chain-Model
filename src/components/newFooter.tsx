...
const Footer: React.FC<FooterProps> = ({ tradeFooter }) => {
  ...
  return (
    <section className={`flex h-fit w-screen flex-col items-center justify-end rounded-tl-[30px] rounded-tr-[30px] ${mode == "dark" ? 'bg-[#070707]' : tradeFooter && selectedTradingCategory == "cefi" ? "shadow shadow-[#71D5E1] bg-gradient-to-bl from-[#71D5E1] to-[#4992E2]" : 'bg-gradient-to-br from-colorSeven-500 to-colorFour-500'} xl:h-fit`} style={{
      ...
      <div className="flex h-fit w-full flex-col items-center justify-center px-20 pt-3 xl:flex-row">
      ...
      <div className="flex h-fit w-full flex-row items-center justify-center px-20 pb-3 pt-1 xl:flex-row">
        <Link href={"/terms_and_conditions"}>
          <h5 className={`interMedium my-0 text-center text-sm ${mode == "dark" ? "text-whiteText-500" : " text-blackText-500"}`}>
            Terms & Conditions
          </h5>
        </Link>
        <div className={`h-[1rem] w-[1px] mx-2 ${mode == "dark" ? "bg-whiteText-500" : " bg-blackText-500"}`}></div>
