...
const IndicesTable = () => {
    ...
    const rows = nexTokens.map((token, index) => ({
        ...
        totalSupply: token.smartContractInfo?.totalSupply ? `${formatToViewNumber({ value: token.smartContractInfo?.totalSupply, returnType: 'string' })}` : '', // Replace with actual data
        change24h: '+5%', // Replace with actual data
        address: reduceAddress(token.tokenAddresses?.[chain]?.[network]?.index?.address as Address),
    }));
        ...
        <Stack width="100%" gap={2} paddingX={{ xs: '2px', lg: 0 }}>
            ...
            <GenericTable
                ...
                autoSizeOptions={{
                    ...
                    outliersFactor: 3,
                    expand: true,
                    columns: isMobile ? ['index', 'price', 'change24h'] : ['index', 'price', 'totalSupply', 'change24h', 'address'],
                }}
            />
        </Stack>
    );
