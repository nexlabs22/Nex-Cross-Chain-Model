...
export function GetDefiPortfolioBalance(swapFromToken: TokenObject, swapToToken: TokenObject) {
	
	const [portfolioValue, setPortfolioValue] = useState<number>()
	const { nexTokens} = useDashboard()
	

	const allowedSymbols = nexTokens.filter((token) => token.smartContractType === 'defi').map((token) => token.symbol)
	const activeTicker = [swapFromToken.symbol, swapToToken.symbol].filter((symbol) => allowedSymbols.includes(symbol))[0]	

	const getPortfolioValue = useCallback(async () => {
		
		const sepoliaPublicClient = getClient('sepolia')
		const activeFactoryAddress = nexTokens.filter((token)=> token.symbol === activeTicker)[0].tokenAddresses?.Ethereum?.Sepolia?.factory?.address
