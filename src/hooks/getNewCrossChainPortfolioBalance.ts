		...
		for (let i = 0; i < Number(totalCurrentList); i++) { 
			...
			if(tokenChainSelector == chainSelectorAddresses.Arbitrum?.Sepolia){
				const crossChainValue = await sepoliaPublicClient.readContract({
					...
					args:[tokenAddresses.WETH?.Ethereum?.Sepolia?.index?.address, tokenAddresses.CRYPTO5?.Ethereum?.Sepolia?.ccip?.address as Address, (wethAmount*Number(tokenMarketShare)/100e18).toFixed(0), 3]
				})
				const tokenValue = await arbitrumSepoliaPublicClient.readContract({
					...
					abi: indexFactoryV2Abi,
					functionName: 'getAmountOut',
					args:[tokenAddresses.CRYPTO5?.Arbitrum?.Sepolia?.ccip?.address, tokenAddresses.WETH?.Arbitrum?.Sepolia?.index?.address, crossChainValue, 3]
				})
